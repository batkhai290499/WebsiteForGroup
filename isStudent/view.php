              ...
              <div class="card-body">
                <form method="post" enctype="multipart/form-data" action="upfile.php">
                  ...
                  <div class="form-group">
                    ...
                    <select name="tutorGroup[]" class="form-control multiple-select" multiple="multiple">
                      <?php
                      require_once '../database.php';
                      $sql = "SELECT tutor.accountID, tutor.tutorName FROM group1 INNER JOIN tutor ON group1.tutorId = tutor.accountID where studentId = " . $_SESSION['accountID'];
                      $rows = query($sql);
                      for ($i = 0; $i < count($rows); $i++) {
                      ?>
                        <div>
